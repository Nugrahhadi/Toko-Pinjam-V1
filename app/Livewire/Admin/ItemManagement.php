...
class ItemManagement extends Component
{
    ...
    public function markReturned(int $rentalId): void
    {
        ...
        if ($rental->status !== 'ongoing') {
            session()->flash('message', 'Hanya status "sedang dipinjam" yang bisa dikembalikan.');
            return;
        }

        DB::transaction(function () use ($rental) {
            // kembalikan stok
            $item = Item::where('id', $rental->item_id)->lockForUpdate()->first();
            if ($item) {
                $item->update(['stock' => (int)$item->stock + (int)$rental->quantity]);
            }
            // ubah status
            $rental->update(['status' => 'returned', 'returned_at' => now()]);
        });

        session()->flash('message', 'Barang ditandai sudah kembali & stok ditambahkan.');
    }
    ...
    public function openEditModal(int $rentalId): void
    {
        ...
        if ($r->status !== 'booked') {
            session()->flash('message', 'Hanya pesanan berstatus "Booked" yang bisa diedit.');
            return;
        }
    ...
    public function updateBooking(): void
    {
        ...
        if ($r->status !== 'booked') {
            $this->addError('form', 'Pesanan tidak bisa diedit karena tidak berstatus "Booked".');
            return;
        }
        ...
        DB::transaction(function () use ($r) {
            ...
            if ($newItemId === $oldItemId) {
                ...
                $oldItem = Item::where('id', $oldItemId)->lockForUpdate()->first();
                $oldItem->increment('stock', $oldQty);

                // Ambil stok dari item baru
                $newItem = Item::where('id', $newItemId)->lockForUpdate()->first();
                if ($newItem->stock < $newQty) {
                    throw new \RuntimeException("Stok untuk barang baru tidak cukup. Tersedia: {$newItem->stock}");
                }
    ...
    public function deleteBooking(int $rentalId): void
    {
        ...
        if ($r->status !== 'booked') {
            session()->flash('message', 'Hanya pesanan berstatus "Booked" yang bisa dihapus.');
            return;
        }
    ...
    public function confirmDelete($id){ $this->deleteItemId = $id; $this->showDeleteModal = true; }
    public function cancelDelete(){ $this->showDeleteModal = false; $this->deleteItemId = null; }

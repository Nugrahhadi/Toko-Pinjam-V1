@section('title', 'Kelola Partner Media')
...
<div>
    ...
    <div class="mb-8 flex flex-col md:flex-row md:items-center md:justify-between">
        <div>
            <h1 class="text-2xl font-bold text-gray-900">Kelola Partner Media</h1>
            <p class="text-gray-600">Atur logo, tautan, dan urutan press rilis di halaman beranda.</p>
        </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10">
        ...
        <div class="bg-white rounded-xl shadow-sm border p-6 md:col-span-2">
            ...
            <p class="text-base font-semibold text-gray-900">Maksimal 8 media akan tampil dan *looping* di beranda.</p>
        </div>
    </div>

    {{-- Daftar Partner Media (Dengan fitur Drag & Drop/Reorder) --}}
    <div class="bg-white rounded-xl shadow-sm border overflow-hidden mb-10">
        ...
        <div class="p-6">
            <div id="partner-sortable-list" class="space-y-3">
                ...
                @empty
                    <p class="px-6 py-6 text-center text-gray-500">Belum ada partner media yang ditambahkan.</p>
                @endforelse
            </div>
        </div>
    </div>
    ...
    @if($showAddModal)
        <div class="fixed inset-0 bg-black/40 flex items-center justify-center z-50" wire:click="$set('showAddModal', false)">
            <div class="bg-white w-full max-w-md rounded-xl p-6" wire:click.stop>
                <h3 class="text-xl font-bold mb-4">Tambah Partner Media</h3>
                ...
                <form wire:submit.prevent="addPartner" class="space-y-4">
                    <div>
                        ...
                        <input type="text" id="mediaName" wire:model="newMedia.name" 
                               class="mt-1 w-full border rounded-md p-2">
                        @error('newMedia.name') <p class="text-red-600 text-sm">{{ $message }}</p> @enderror
                    </div>
                    <div>
                        <label for="mediaUrl" class="block text-sm font-medium text-gray-700">URL Artikel/Website (Opsional)</label>
                        <input type="url" id="mediaUrl" wire:model="newMedia.url" 
                               class="mt-1 w-full border rounded-md p-2">
                        @error('newMedia.url') <p class="text-red-600 text-sm">{{ $message }}</p> @enderror
                    </div>
                    <div>
                        <label for="mediaLogo" class="block text-sm font-medium text-gray-700">Logo Media (Maks 1MB, PNG/JPG)</label>
                        <input type="file" id="mediaLogo" wire:model="newMedia.logo" 
                        ...
                        @error('newMedia.logo') <p class="text-red-600 text-sm">{{ $message }}</p> @enderror

                        @if ($newMedia['logo'])
                            <p class="text-xs text-gray-500 mt-2">Pratinjau:</p>
                            <img src="{{ $newMedia['logo']->temporaryUrl() }}" class="w-16 h-16 object-contain border rounded p-1 mt-1">
                        @endif
                    </div>
    ...
    <div class="fixed inset-0 bg-black/40 flex items-center justify-center z-50" wire:click="$set('partnerToEditId', null)">
        <div class="bg-white w-full max-w-md rounded-xl p-6" wire:click.stop>
            <h3 class="text-xl font-bold mb-4">Edit Partner Media</h3>
            
            <form wire:submit.prevent="updatePartner" class="space-y-4">
                <div>
                    <label for="editName" class="block text-sm font-medium text-gray-700">Nama Media</label>
                ...
                <div>
                    <label for="editUrl" class="block text-sm font-medium text-gray-700">URL Artikel/Website (Opsional)</label>
                ...
                <div>
                    <label for="editLogo" class="block text-sm font-medium text-gray-700">Ganti Logo Media (Maks 1MB)</label>
